...
if (isset($_POST['relasi_ubah'])) {
    $kode_diagnosa = isset($_POST['kode_diagnosa']) ? trim($_POST['kode_diagnosa']) : '';
    $kode_gejala = isset($_POST['kode_gejala']) ? trim($_POST['kode_gejala']) : '';
    ...
    if (empty($kode_diagnosa) || empty($kode_gejala) || empty($mb) || empty($md)) {
        ...
        $kombinasi_ada = $db->get_row("SELECT * FROM tb_relasi 
        ...
        if ($kombinasi_ada) {
            ...
            $db->query("UPDATE tb_relasi 
                       SET kode_diagnosa='$kode_diagnosa', 
...
<?php require "sidebar-navbar/navbar.php"; ?>
            ...
            <form method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="md" class="form-input"
                           value="<?= htmlspecialchars($row->md) ?>"
                           placeholder="Masukkan nilai MD">
                </div>

                <div class="tombol-grup">
                    <button type="submit" name="relasi_ubah" class="tombol tombol-primer">
                        Simpan
                    </button>
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            // Validasi MB dan MD antara 0 dan 1
            if (mb < 0 || mb > 1 || md < 0 || md > 1) {
                e.preventDefault();
                alert('MB dan MD harus bernilai antara 0 dan 1');
            }
        });
