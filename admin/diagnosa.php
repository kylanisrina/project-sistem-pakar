            ...
            <div class="tools">
                <div>
                    <form method="get">
                        ...
                        <input class="pencarian" type="text" placeholder="Cari diagnosa..." 
                               name="q" value="<?= isset($_GET['q']) ? htmlspecialchars($_GET['q']) : '' ?>" />
                    </form>
                </div>
                <a href="diagnosa_tambah.php" class="btn-primary">
                    Tambah Data
                </a>
            </div>
            ...
            <table class="tabel">
                ...
                <tbody>
                    ...
                    if ($rows) :
                        foreach ($rows as $row) : ?>
                            <tr>
                                <td><?= htmlspecialchars($row->kode_diagnosa) ?></td>
                                <td><?= htmlspecialchars($row->nama_diagnosa) ?></td>
                                <td><?= htmlspecialchars(get_words($row->keterangan, 20)) ?></td>
                                <td class="aksi">
                                    <a href="diagnosa_ubah.php?ID=<?= urlencode($row->kode_diagnosa) ?>" >
                                       <img src="images/edit.png" width="30" height="25" style="margin-top: 18px;" alt="Edit">
                                    </a>
                                    <a href="aksi.php?act=diagnosa_hapus&ID=<?= urlencode($row->kode_diagnosa) ?>" 
